...
const DeedsTable = ({ sectionTitle = "Description of Documents Scrutinized", tableType = "table", copyFromTableType }: DeedsTableProps) => {
  ...
  const handleCopyFromPreviousTable = async () => {
    ...
    try {
      ...
      // Insert deeds sequentially to preserve order
      for (const sourceDeed of sourceDeedsData) {
        const newDeed = {
      ...
      toast.success(`Copied ${sourceDeedsData.length} deed(s) successfully`);
    } catch (error) {
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="border-b border-border">
                  <th className="p-3 text-left text-sm font-semibold bg-white">Sno</th>
                  <th className="p-3 text-left text-sm font-semibold bg-white">Date</th>
                  <th className="p-3 text-left text-sm font-semibold bg-white">D.No</th>
                  <th className="p-3 text-left text-sm font-semibold bg-white">Particulars of Deed</th>
                  <th className="p-3 text-left text-sm font-semibold bg-white">Nature of Doc</th>
